...
from qdots_qll.distributions import update_weights
...
def pls_resample(key, distribution):
    ...
    key, new_weights, new_particles_locations = (
        resample_result["key"], resample_result["weights"], resample_result["particles_locations"],)
...
def initialize_carry(key):
    ...
    particles_locations = tfp.distributions.TruncatedNormal(loc=mus, scale=sigmas, low=boundaries[:, 0],
                                                            high=boundaries[:, 1]).sample(seed=subkey,
                                                                                          sample_shape=no_particles)
...
def f_scan(carry, _):
    ...
    t = eqx.filter_jit(expdesign.generate_time)(key=subkey, particles_locations=pdist.particles_locations,
                                                weights=pdist.weights,

                                                model=model, prob_initial_state=p_initial_state,
                                                prob_measurement_basis=p_measurement_basis, )

    key, subkey = jax.random.split(key)
    chosen_initial_state = jax.random.choice(subkey, jnp.arange(no_initial_states), p=p_initial_state)

    key, subkey = jax.random.split(key)
    chosen_basis = jax.random.choice(subkey, jnp.arange(no_measurement_basis), p=p_measurement_basis)
    ...
    return (key, pdist, p_initial_state, p_measurement_basis), (
        outcome, t, pdist.ev(), pdist.cov(), p_initial_state, p_measurement_basis,)
...
boundaries = jnp.array([[0.1, 0.5], [0.1, 0.5], [0.01, 0.2], [-0.5, -0.01], ])
...
print("Estimating compilation and iteration time ...")
...
print("Starting runs... ")
_, results = jax.lax.scan(f_scan_mapped, init=initial_carries, xs=None, length=no_max_iterations)
